...
def main(_):
    ...
    logging.info(
        ...
        Finetuning Data Mix: {FLAGS.config.dataset_kwargs["oxe_kwargs"]["data_mix"]}
    ...
    FLAGS.config.dataset_kwargs.batch_size = FLAGS.config.batch_size // jax.process_count()
    ...
    elif FLAGS.config.modality == "language_conditioned":
    ...
    if "rollout_kwargs" in FLAGS.config:
        rollout_callback = RolloutVisualizationCallback(
            text_processor=text_processor,
            unnormalization_statistics=train_data.dataset_statistics["action"],
